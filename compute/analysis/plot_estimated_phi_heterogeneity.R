plot_estimated_phi_heterogeneity <- function(param, save = F,
                     pch = 19, cex = 0.2, each_x = 2,
                     each_y = 0.01, alpha = 0.7,
  ...
  add_empty_plot_with_grid(x_data = 10:49,
                           y_data =  seq(0, 0.25, length.out = 40),
                           x_label_text = "Age",
                           y_label_text =  "f(x)",
                           margin_y = margin_y,
                           margin_x = margin_x)
  ...
  points(10:50, mean_vals[[i]], col="firebrick2", pch = i+1, cex = 1)
  lines(10:50, mean_vals[[i]], col="firebrick2", lwd = 1, lty = 1)
  }
  
  for (i in 1:length(mean_vals)){
  polygon(x = c(10:50, rev(10:50)),
          y = c(as.numeric(int_inf[[i]]),rev(as.numeric(int_sup[[i]]))),
          col = adjustcolor("firebrick2", alpha.f = 0.20), border = NA)
  }
  # Legend
  op <- par(family = "sans")
  
  legend(legend = c("No heterogeneity model",
                    "Heterogeneity Model (Var 0.0015)",
                    "Heterogeneity Model (Var 0.006)",
                    "Heterogeneity Model (Var 0.014)"),
         lwd = c(1,1,1,1), col = "firebrick2", lty = c(1,1,1,1),
         pch = c(2, 3, 4,5),
  ...
  if(save){
    ...
    p <- recordPlot()
    pdf(file.path(save_path, paste0("estimated","_","phi_h",".pdf")), width=7, height=7) 
    print(p)
    dev.off()
  }
}
